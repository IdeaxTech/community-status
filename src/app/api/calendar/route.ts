...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  return NextResponse.json(await getCalendarEvents(year, month));
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (!isValidTime(time)) return NextResponse.json({ error: "time must be HH:MM" }, { status: 400 });
  await addCalendarEvent(date, title, time ?? undefined);
...
export async function PUT(req: NextRequest): Promise<NextResponse> {
  ...
  const title = typeof body.title === "string" ? body.title.trim().slice(0, 100) : "";
  const time = parseTime(body.time);
  if (id === null) return NextResponse.json({ error: "valid id is required" }, { status: 400 });
  if (!title) return NextResponse.json({ error: "title is required" }, { status: 400 });
  if (!isValidTime(time)) return NextResponse.json({ error: "time must be HH:MM" }, { status: 400 });
  const updated = await updateCalendarEvent(id, title, time);
